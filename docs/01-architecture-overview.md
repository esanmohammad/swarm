...
`packages/cli/bin/swarm.ts` registers **60+ commands** across four waves of development:
...
**Wave 2 — Quick Workflows, Automation & Security**

| Command | Description |
|---------|-------------|
| `fix` | Fix a GitHub issue or local error with an AI engineer |
| `spike` | Run a technical feasibility spike |
| `review` | AI code review for a PR, branch, or working tree |
| `refactor` | Two-phase analyst → engineer refactoring |
| `simplify` | Find and remove complexity, dead code, and duplication |
| `ci` | Run the full pipeline in lean CI mode |
| `learn` | Analyze codebase conventions and inject them into agents |
| `memory` | Manage project memory (decisions, conventions, context) |
| `explain` | Generate human-readable explanations of code |
| `stats` | Show pipeline statistics and health summary |
| `watch` | Watch file changes and run targeted tests continuously |
| `babysit-prs` | Continuously review open PRs and post AI feedback |
| `deploy` | AI-driven deployment with environment gating and rollback |
| `migrate` | Database migration via AI with ORM-awareness |
| `server` | Long-running job server with REST API and budget management |
| `autopilot` | Daemon that turns labeled GitHub issues into PRs |
| `test-gen` | Retroactively generate tests for untested code |
| `deps` | Intelligent dependency update with safety checks |
| `risk` | Regression risk scoring for a diff or PR |
| `incident` | Automated production incident response |
| `pr` | Smart PR creation with code ownership awareness |
| `health` | Continuous codebase health monitoring |
| `benchmark` | Performance regression detection |
| `pm` | Project management integration (Jira/Linear/GitHub Projects) |
| `secure` | OWASP static security scanning |
| `secrets` | Secret and credential leak detection |
| `supply-chain` | Supply chain integrity checks |
| `sandbox` | Configure and inspect tool sandboxing policies |
| `prompt-guard` | Detect and block prompt injection attempts |
| `fingerprint` | Identify AI-generated code in the codebase |
| `monitor` | Runtime anomaly detection and behavior monitoring |

**Wave 3 — Autonomous Employee**

| Command | Description |
|---------|-------------|
| `inbox` | Triage GitHub notifications, PRs, and issues |
| `standup` | Generate async daily standup reports |
| `journal` | Log architectural decisions and context |
| `scope` | Scope ambiguous feature requests before planning |
| `context` | Build and serve codebase context index |
| `pair` | AI pair-programming session |
| `delegate` | Spawn parallel engineers from a backlog |
| `report` | Generate project health and progress reports |
| `team` | Simulate team activity and surface conflicts |
| `retro` | Retrospective analysis and velocity insights |

**Wave 4 — Autonomous Engineering Organization**

| Command | Description |
|---------|-------------|
| `own` | Surface ownership registry with SLO tracking |
| `architect-review` | Static architectural analysis and coupling detection |
| `onboard` | Guided onboarding experience with progress tracking |
| `mentor` | Q&A, code review coaching, and explanation mode |
| `roadmap` | Multi-phase roadmap planning with critical path |
| `system` | Cross-repo service graph and contract validation |
| `slo` | Service Level Objective management and automated checks |
| `evolve` | Tech debt scanning and automated fix suggestions |
| `forecast` | Velocity, risk, cost, and health predictions |
| `compliance` | Automated SOC 2, HIPAA, GDPR, PCI DSS checks |

### Prompts Directory

The `prompts/` directory contains **35 bundled persona system prompts** (5 personas × 7 tech stacks):

| Persona | Filenames | Artifact | Role Boundary |
|---------|-----------|----------|---------------|
| Analyst | `analyst-{react,node,go,python,rust,swift,custom}.md` | REQUIREMENTS.md | No architecture, no code, no tasks |
| Architect | `Architect-{React,Node,Go,Python,Rust,Swift,Custom}.md` | SPEC.md | No code, no task breakdown |
| Lead | `Software-lead-{react,node,go,python,rust,swift,custom}.md` | TASKS.md | No code, no architecture redesign |
| Engineer | `Software-engineer-{react,node,go,python,rust,swift,custom}.md` | Code | Full access to all tools |
| Tester | `test-engineer-{react,node,go,python,rust,swift,custom}.md` | TESTPLAN.md | No implementation code, only test plans |
...
| `autopilot-start` | label, interval, maxConcurrent, budget, dryRun | Start autopilot daemon |
| `autopilot-stop` | -- | Stop autopilot daemon |
| `get-surfaces` | -- | Request surface ownership state |
| `run-arch-review` | focus? | Trigger architecture review |
| `run-forecast` | -- | Trigger forecast computation |
| `run-compliance` | frameworks? | Trigger compliance check |
| `slo-check` | -- | Run SLO checks against current metrics |
| `evolve-scan` | -- | Scan for tech debt |
| *(+ 20 more Wave 2–4 commands)* | | |
...
Swarm persists all feature state to `.swarm/` alongside the core `state.json`:

| File | Written by | Contents |
|------|-----------|----------|
| `state.json` | StateManager | Full PipelineState including agents, stages, cost, MayDay |
| `state.json.bak` | StateManager | Backup before each write for crash recovery |
| `config.yaml` | `swarm init` | Project configuration (stack, model, ports, etc.) |
| `guardrails.yaml` | User / `swarm evaluate` | Custom artifact validation rules |
| `audit.jsonl` | AuditLog | Structured event log (newline-delimited JSON) |
| `conventions.json` | `swarm learn` | Extracted codebase conventions |
| `memory.json` | `swarm memory` | Project memory entries with TTL |
| `slos.json` | `swarm slo` | SLO definitions and error budget state |
| `debt.json` | `swarm evolve` | Current tech debt scan results |
| `debt-history.jsonl` | `swarm evolve` | Append-only debt trend history |
| `forecast.json` | `swarm forecast` | Velocity, risk, and health predictions |
| `compliance-report.json` | `swarm compliance` | Latest compliance check results |
| `autopilot-state.json` | `swarm autopilot` | Daemon state, processed issues, queue |
| `surfaces.yaml` | `swarm own` | Surface ownership definitions |
| `surfaces-status.json` | `swarm own check` | Current health per surface |
| `arch-review.json` | `swarm architect-review` | Latest architecture analysis results |
| `roadmap.json` | `swarm roadmap` | Phases, milestones, critical path |
| `system/graph.json` | `swarm system` | Cross-repo service dependency graph |
| `onboard-progress.json` | `swarm onboard` | Per-user onboarding step progress |
| `sandbox.yaml` | `swarm sandbox` | Tool sandboxing policy configuration |
| `watch-status.json` | `swarm watch` | Current watch loop state and last run |
| `health.json` | `swarm health` | Codebase health metrics |
| `pm-sync.json` | `swarm pm` | PM integration sync state and ticket map |

---

## 9. Feature Wave Summary

Swarm was built in four waves, each expanding the surface area:

```mermaid
graph TB
    subgraph W1["Wave 1 — Core Pipeline"]
        W1A["5-stage pipeline<br/>(Analyst → Tester)"]
        W1B["MayDay autonomous mode"]
        W1C["Dashboard + WebSocket"]
        W1D["Guardrails + Cost tracking"]
    end

    subgraph W2["Wave 2 — Quick Workflows & Automation"]
        W2A["Single-purpose workflows<br/>(fix, spike, review, ci…)"]
        W2B["Continuous monitoring<br/>(watch, babysit-prs, health)"]
        W2C["Infrastructure<br/>(deploy, migrate, server)"]
        W2D["Hands-free automation<br/>(autopilot, test-gen, deps)"]
        W2E["Security suite<br/>(secure, secrets, supply-chain…)"]
    end

    subgraph W3["Wave 3 — Autonomous Employee"]
        W3A["Daily workflow<br/>(inbox, standup, journal)"]
        W3B["Planning tools<br/>(scope, context, roadmap)"]
        W3C["Collaboration<br/>(pair, delegate, team, retro)"]
    end

    subgraph W4["Wave 4 — Autonomous Organization"]
        W4A["Ownership & reliability<br/>(own, slo, health)"]
        W4B["Architecture intelligence<br/>(architect-review, system)"]
        W4C["Engineering culture<br/>(onboard, mentor, evolve)"]
        W4D["Forecasting & compliance<br/>(forecast, compliance)"]
    end

    W1 --> W2 --> W3 --> W4
```

See the documentation index in `docs/README.md` for detailed docs on each wave.
